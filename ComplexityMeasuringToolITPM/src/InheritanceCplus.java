	...
	public static ArrayList<InheritanceComplexity> inheritanceCi(ArrayList<String> line,int NoClass,int OneClass,int TwoClass ,int ThreeClass,int MoreClass) {
		...
		for (int i = 0; i < line.size(); i++) {
			...
			for (int r = 0; r < wordsForVariables.length; r++) {
				...
				if (wordsForVariables.length == 4 ) {
					System.out.println("5iiiiii");

					if (wordsForVariables[r].contains("class") && wordsForVariables[r + 1].contains(":")
							&& wordsForVariables[r + 2].contains("public")) {

						String indexOfcolon = wordsForVariables[r + 1].replaceAll("\\:", "");// catch the index after
																								// the barcketss
																								// byindexOf rami
						...
						String indexOfBrack = wordsForVariables[r + 3].replaceAll("\\{", "");
						...
						for (int j = 0; j < line.size(); j++) {
							...
							for (int p = 0; p < words.length; p++) {
								...
								if (words.length == 4 ) {
									...
									if (words[p].contains("class") && words[p + 1].contains(":")
											&& words[p + 2].contains("public")) {

										String SecondindexOfBrackTwo = words[p + 3].replaceAll("\\{", "");// catch the
																											// index
																											// after the
																											// barcketss
																											// byindexOf
